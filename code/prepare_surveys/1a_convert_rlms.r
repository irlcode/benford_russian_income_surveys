...
# Init an empty object
rlms_household_representative <- data.table()

for(f in survey_files_representative) {
		...
		# Variables of interest
		varnames <- names(temp)
		varnames_interest <- varnames[ grepl("(^[A-z]{1,2}id_h)", varnames, ignore.case = T)] 
		temp <- temp[, c(varnames_interest), with = F]
		...
		rlms_household_representative <- rbind(rlms_household_representative, temp, fill = T)
		
}

# Mark whether surveyed unit is representative
rlms_household_representative[, representative := 1]
rlms_household <- merge(rlms_household, rlms_household_representative, by = c("id_h", "id_w"), all.x = T, all.y = F, sort = F)
...
for(f in survey_files) {
		...
		if( any(grepl("h7.1", varnames_new, ignore.case = T)) ) {

			temp[, h7.1 := as.numeric(h7.1)]

		}

		if( any(grepl("h7.2", varnames_new, ignore.case = T)) ) {

			temp[, h7.2 := as.numeric(h7.2)]

		}

		if( any(grepl("h8a", varnames_new, ignore.case = T)) ) {

			temp[, h8a := as.numeric(h8a)]

		}

		if( any(grepl("h8b", varnames_new, ignore.case = T)) ) {

			temp[, h8b := as.numeric(h8b)]
...
# Init an empty object
rlms_individual_representative <- data.table()
...
for(f in survey_files_representative) {
		...
		# Variables of interest
		varnames <- names(temp)
		varnames_interest <- varnames[ grepl("(^[A-z]{1,2}id_h)|(^idind$)", varnames, ignore.case = T)] 
		temp <- temp[, c(varnames_interest), with = F]
		...
		rlms_individual_representative <- rbind(rlms_individual_representative, temp, fill = T)
		
}

# Mark whether surveyed unit is representative
rlms_individual_representative[, representative := 1]
rlms_individual <- merge(rlms_individual, rlms_individual_representative, by = c("id_h", "id_w", "idind"), all.x = T, all.y = F, sort = F)
